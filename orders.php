...
<header>
   ...
   <div class="container">
      ...
      <a class= "nav-link text-light d-flex" href="cart.php" class="cart "> <span class="cart-digit"><?php echo $row_count; ?></li>
        ...
        <div class="container">
        <a href="products.php" class="contin">Продолжить покупки</a>
  <br><br>
  <center>
...
$sql = "SELECT name, number, email, method, street, total_product, total_price FROM `order`";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["name"]. "</td><td>" . $row["number"]. "</td><td>" . $row["email"]. "</td><td>" . $row["method"]. "</td><td>" . $row["street"]. "</td><td>" . $row["total_product"]. "</td><td>" . $row["total_price"]. "</td></tr>";
  }
  echo "</table>";
} else {
  echo "0 results";
